...
export const RankingView = ({ ranking, players, onMatchClick, onBack, onAddDivision, onUpdateRanking, isAdmin }: Props) => {
  ...
  const copyToClipboard = () => {
    const url = ranking.publicUrl || window.location.href;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-900">{ranking.nombre}</h2>
            <div className="flex items-center gap-2 text-sm text-gray-500">
                <span className={`uppercase font-medium ${ranking.status === 'activo' ? 'text-green-600' : 'text-gray-500'}`}>{ranking.status}</span>
                <span>•</span>
        ...
        <div className="flex gap-2">
          {isAdmin && onUpdateRanking && (
              <Button onClick={handleOpenPromotionModal} className="bg-indigo-600 hover:bg-indigo-700 text-white flex items-center gap-2 text-sm">
                  <Flag size={16} /> Finalizar Fase
              </Button>
          )}
          <Button variant="secondary" className="!p-2 text-primary" onClick={copyToClipboard} title="Copiar URL Pública">
        ...
        {ranking.divisions.sort((a,b) => a.numero - b.numero).map(div => (
            <button
                ...
                className={`px-4 py-2 rounded-t-lg font-medium text-sm transition-colors whitespace-nowrap ${
                    activeDivisionId === div.id && activeTab !== 'global'
                    ? 'bg-white border-b-2 border-primary text-primary shadow-sm' 
                    : 'bg-gray-50 text-gray-600 hover:bg-gray-100'
                }`}
            >
                ...
                <table className="w-full text-sm text-left">
                  <thead className="bg-gray-50 text-gray-500 font-medium border-b">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-center">Dif Sets</th>
                      <th className="px-6 py-3 text-center">Dif Juegos</th>
                      <th className="px-6 py-3 text-center">% Vic</th>
                    </tr>
                  </thead>
                    ...
                    {globalStandings.map((row) => {
                      ...
                      return (
                        <tr key={row.playerId} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-4 text-center font-bold text-gray-400">{row.pos}</td>
                          <td className="px-6 py-4 font-medium text-gray-900">
                             {player.nombre} {player.apellidos}
                          </td>
                          <td className="px-6 py-4 text-center text-gray-600">{row.pj}</td>
                          <td className="px-6 py-4 text-center font-bold text-primary">{row.pts}</td>
                          <td className="px-6 py-4 text-center text-green-600">{row.pg}</td>
                          <td className="px-6 py-4 text-center text-red-600">{row.pj - row.pg}</td>
                          <td className="px-6 py-4 text-center text-gray-600">{row.setsDiff > 0 ? `+${row.setsDiff}` : row.setsDiff}</td>
                          <td className="px-6 py-4 text-center text-gray-600">{row.gamesDiff > 0 ? `+${row.gamesDiff}` : row.gamesDiff}</td>
                ...
                {standings.map((row) => {
                  ...
                  return (
                    <tr key={row.playerId} className={`hover:bg-gray-50 transition-colors ${rowClass}`}>
                      <td className="px-6 py-4 text-center font-bold text-gray-700">{row.pos}</td>
                      <td className="px-6 py-4 font-medium text-gray-900">
                        {player.nombre} {player.apellidos}
                      </td>
                      <td className="px-6 py-4 text-center text-gray-600">{row.pj}</td>
                      <td className="px-6 py-4 text-center font-bold text-primary">{row.pts}</td>
                      <td className="px-6 py-4 text-center font-medium text-gray-700">{winrate}%</td>
                      <td className="px-6 py-4 text-center text-gray-600">{row.setsDiff > 0 ? `+${row.setsDiff}` : row.setsDiff}</td>
                      <td className="px-6 py-4 text-center text-gray-600">{row.gamesDiff > 0 ? `+${row.gamesDiff}` : row.gamesDiff}</td>
                    </tr>
                  );
      ...
      {activeTab === 'matches' && activeDivision && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {activeDivision.matches.map((m) => {
            ...
            const p1Names = `${p1.nombre} & ${p2.nombre}`;
            const p2Names = `${p3.nombre} & ${p4.nombre}`;
              ...
              >
                ...
                <div className="space-y-3">
                    <div className="flex justify-between items-center">
                        <span className={`text-sm font-medium ${m.points.p1 > m.points.p2 ? 'text-gray-900 font-bold' : 'text-gray-600'}`}>
                            {p1Names}
                        </span>
                        {m.status === 'finalizado' && (
                            <span className="font-mono font-bold text-gray-800">
                                {m.score?.set1.p1} {m.score?.set2?.p1} {m.score?.set3?.p1}
                            </span>
                        )}
                    </div>
                    <div className="flex justify-between items-center">
                        <span className={`text-sm font-medium ${m.points.p2 > m.points.p1 ? 'text-gray-900 font-bold' : 'text-gray-600'}`}>
                            {p2Names}
                        </span>
                        {m.status === 'finalizado' && (
                            <span className="font-mono font-bold text-gray-800">
                                {m.score?.set1.p2} {m.score?.set2?.p2} {m.score?.set3?.p2}
                            </span>
                        )}
                    </div>
                </div>
