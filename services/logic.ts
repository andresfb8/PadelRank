...
export function calculateMatchPoints(
  ...
  if (!isIncomplete) {
    ...
    if (p1Sets > p2Sets) return { points: { p1: 4, p2: 0 }, finalizationType: 'completo', description: 'Victoria Completa' }; 
    if (p1Sets === 2 && p2Sets === 0) return { points: { p1: 4, p2: 0 }, finalizationType: 'completo', description: 'Victoria 2-0' };
    if (p1Sets === 2 && p2Sets === 1) return { points: { p1: 3, p2: 1 }, finalizationType: 'completo', description: 'Victoria 2-1' };
    if (p1Sets === 1 && p2Sets === 2) return { points: { p1: 1, p2: 3 }, finalizationType: 'completo', description: 'Derrota 1-2' };
    return { points: { p1: 0, p2: 4 }, finalizationType: 'completo', description: 'Derrota 0-2' };
  }
  ...
  if (p1WonS1) {
    if (p1LeadsS2) {
      return { points: { p1: 4, p2: 0 }, finalizationType: 'victoria_incompleta', description: 'Victoria Incompleta (Ventaja)' };
    } else {
      if (diffS2 >= 3) {
         return { points: { p1: 2, p2: 2 }, finalizationType: 'empate_diferencia', description: 'Empate por diferencia' };
      } else {
         return { points: { p1: 4, p2: 0 }, finalizationType: 'victoria_incompleta', description: 'Victoria Incompleta (Rival no remonta)' };
      }
    }
  } 
  // SCENARIO B: P1 Lost Set 1
    ...
    if (p1LeadsS2) {
      if (diffS2 >= 3) {
        ...
        return { points: { p1: 0, p2: 4 }, finalizationType: 'derrota_incompleta', description: 'Derrota Incompleta (Dif < 3)' };
      }
    } else {
      return { points: { p1: 0, p2: 4 }, finalizationType: 'derrota_incompleta', description: 'Derrota Incompleta' };
    }
  }
}
